...
static __always_inline void cleanup_conn(conn_tuple_t* tup) {
    ...
    if (batch_ptr == NULL) {
        log_debug("ERR(cleanup_conn): no batch for cpu: %u\n", cpu);
        return;
    }

    // TODO: Can we turn this into a macro based on TCP_CLOSED_BATCH_SIZE?
    log_debug("cleanup_conn: cpu: %u pos: %u\n", cpu, batch_ptr->pos);
...
static __always_inline void flush_conn_close_if_full(struct pt_regs * ctx) {
    ...
    batch_t* batch_ptr = bpf_map_lookup_elem(&conn_close_batch, &cpu);
    if (!batch_ptr) {
        log_debug("ERR(flush_conn_close_if_full): no batch for cpu: %u\n", cpu);
        return;
    }

    log_debug("flush_conn_close_if_full: cpu: %u pos: %u\n", cpu, batch_ptr->pos);
