...
func (ns *networkState) addDNSStats(id string, conns []ConnectionStats) {
	...
	for i := range conns {
		...
		if dnsStatsByDomain, ok := ns.clients[id].dnsStats[key]; ok {
			if ns.collectDNSDomains {
				conn.DNSStatsByDomainByQueryType = make(map[*stringintern.Value]map[dns.QueryType]dns.Stats)
			} else {
	...
	// flush the DNS stats
	ns.clients[id].dnsStats = make(dns.MapByKeyNameType)
}
...
func getDeepDNSStatsCount(stats dns.MapByKeyNameType) int {
	var count int
...
func (ns *networkState) storeDNSStats(stats dns.MapByKeyNameType) {
	for _, client := range ns.clients {
			...
			for domain, statsByQtype := range statsByDomain {
				for qtype, dnsStats := range statsByQtype {
					...
					if _, ok := client.dnsStats[key]; !ok {
						...
						client.dnsStats[key] = make(map[*stringintern.Value]map[dns.QueryType]dns.Stats)
					}
