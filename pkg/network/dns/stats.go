...
type dnsPacketInfo struct {
	...
	question      *stringintern.Value // only relevant for query packets
	queryType     QueryType
}
...
func (d *dnsStatKeeper) ProcessPacketInfo(info dnsPacketInfo, ts time.Time) {
	...
	if !ok {
		allStats = make(map[*stringintern.Value]map[QueryType]Stats)
	}
...
func (d *dnsStatKeeper) GetAndResetAllStats() MapByKeyNameType {
	d.mux.Lock()
	defer d.mux.Unlock()
	ret := d.stats // No deep copy needed since `d.stats` gets reset
	d.stats = make(MapByKeyNameType)
...
func (d *dnsStatKeeper) Snapshot() MapByKeyNameType {
	d.mux.Lock()
	defer d.mux.Unlock()

	snapshot := make(MapByKeyNameType)
	for key, statsByDomain := range d.stats {
		snapshot[key] = make(map[*stringintern.Value]map[QueryType]Stats)
